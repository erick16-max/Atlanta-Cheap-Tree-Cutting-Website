...
export async function POST(req) {
  try {
    const { to, subject, time, date, fullname, address} = await req.json();
    const imageLogoUrl =
      "https://atlanta-cheap-tree-cutting-website.vercel.app/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Flogo.0d91f4e9.png&w=256&q=75";
    ...
    const response = await resend.emails.send({
      ...
      html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;">
            ...
            <h2 style="color: #111; text-align: center;">Hello 👋,${fullname}</h2>
            <h2 style="color: #333; text-align: center;">${subject}</h2>
            <h4 style="color: #333; text-align: center;">Survey Date:<span style="font-size: 16px; color: #555">${date}</span></h4>
            
            <h4 style="color: #333; text-align: center;">Survey Time:<span style="font-size: 16px; color: #555">${time}</span></h4>
            <h4 style="color: #333; text-align: center;">Survey Location:<span style="font-size: 16px; color: #555">${address}</span></h4>
