...
export default function UpdateStatusModal({ open, setOpen }) {
    ...
    const {bookingObj, selectedItemId, setBookingTableData} = useContext(AppContext)
    ...
    const handleUpdateStatus = async() => {
        try {
            setLoading(true)
            const response = await updateBookingStatus(selectedItemId,status, setBookingTableData)
            if(response === "success"){
                setOpen(false)
            }
        } catch (error) {
      ...
      >
        ...
        <Stack gap={2} mt={3}>
          ...
          <Button
            ...
            disabled={loading}
          >
            {loading ? <CircularProgress size={20} thickness={4} /> : "Update Status"}
          </Button>
        </Stack>
